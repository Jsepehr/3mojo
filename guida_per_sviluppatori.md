...
**Due ottimizzazioni sul peso di calcolo** (il rilevamento di nuovi hotspot, con ≥3 persone e triple da provare, è l'unica parte potenzialmente costosa qui dentro):
1. **Partizionamento spaziale per fascia di latitudine**: la ricerca combinatoria di nuove triple non gira più su *tutte* le sessioni idonee del server, ma raggruppate in fasce larghe quanto `clusterRadiusMeters` — due sessioni in fasce non adiacenti sono per forza più lontane di 100m, quindi non potrebbero mai passare `_allPairwiseWithin` insieme, e non vengono nemmeno provate. Una tripla valida può scavalcare al massimo una fascia di confine (mai due), quindi cercare in (fascia-1, fascia, fascia+1) per ogni fascia occupata la trova comunque — può essere trovata da più fasce, ma `_wouldOverlapAnotherHotspot` impedisce comunque il doppione, quindi il lavoro ripetuto è innocuo. Scala con la densità locale, non con quante sessioni sono online in tutto il server.
2. **Rilevamento su un timer proprio, non a ogni presenza**: `HotspotStore.detectAndRefresh` non gira più dentro `ConnectionHub.broadcastNearbyUpdates` (chiamato a ogni singolo aggiornamento di posizione di chiunque sia online — con molti utenti connessi, potenzialmente decine di volte al secondo). Ora gira su un `Timer.periodic` proprio (`ConnectionHub.startHotspotDetection`, 15s di default, avviato una volta da `routes/ws.dart` come `SessionStore.startAutoPurge`), del tutto ragionevole dato che un hotspot si forma in 15 minuti e vive un'ora — qualche secondo di ritardo nella *scoperta* non conta nulla. `broadcastNearbyUpdates` resta invece istantaneo a ogni presenza: riflette sempre lo stato hotspot più recente già noto, senza dover ricalcolare nulla lui stesso.

**Provare hotspot/vicinanze senza dispositivi reali** ([tool/simulate_fake_devices.dart](server/tool/simulate_fake_devices.dart), `dart run tool/simulate_fake_devices.dart` col server già acceso in locale): apre 20 vere connessioni WebSocket verso il server (non un mock — lo stesso protocollo che userebbe l'app), ognuna con una posizione finta diversa. Disposizione pensata apposta: due gruppi da 4+1 (i 4 già si vedono tra loro fin da subito con la regola normale, il quinto è deliberatamente troppo lontano dal nucleo per i 100m base ma dentro ai 200m dell'hotspot — è quello da guardare per vedere l'effetto vero comparire dopo 15 minuti, non solo "un hotspot esiste da qualche parte"), più 5 coppie isolate lontanissime da tutto il resto per la sola visibilità normale a due. Stampa la lista "vicinanze" di ogni dispositivo finto solo quando cambia composizione, così non serve seguire un log a piena velocità.
